...
void joyCallback(const sensor_msgs::Joy &controller) {
    if (controller.axes[5] < 0 or controller.axes[1] < 0 or controller.axes[2] < 0 or controller.axes[3] < 0) {
        flag_plus = 1;
    }
    if (controller.buttons[5] or controller.buttons[1] or controller.buttons[2] or controller.buttons[3]) {
        flag_plus = 3;
    }
...
void checkJoint() {
    for (int i < 0; i < flag_joint_check.size(); ++i) {
        if (joint1_pos.data + 0.5 > prev_value && joint1_pos.data - 0.5 < prev_value) {
...
void angleCal() {
    ...
    if (flag _designation) {
        ...
        flag_plus == 1 ? now_status = FORWARD_ALL : flag_plus == 3 ? now_status = REVERSE_ALL : now_status = BRAKE;
    } else if (flag_brake) {
        now_status = BRAKE;
    } else {
        flag_plus == 1 ? now_status = FORWARD : flag_plus == 3 ? now_status = REVERSE : now_status = BRAKE;
    }
...
int main(int argc, char **argv) {
    ...
    while (ros::ok()) {
        ...
        cout << "check = " << check << " " << "call = " << call << endl;
        ...
        ROS_INFO("Joint1(%s)= %f", jtp0.joint_names[0].c_str(), jtp0.points[0].positions[0]);
    }
